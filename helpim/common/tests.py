...
class ImporterTestCase(TestCase):
    ...
    def test_import_questionnaires(self):
        ...
        unanswered_questionnaire = HIQuestionnaire(
            ...
            fields=[
                HIQuestionnaireField(id=20, label='city?', type=1, choices=None, visible=True), # text
                HIQuestionnaireField(id=21, label='color?', type=5, choices='red,blue,green,yellow,pink', visible=True), # checkbox_multiple
                HIQuestionnaireField(id=22, label='double', type=15, choices='one(),two(A,B,C),three', visible=True), # doubledrop
            ],
            submissions=[],
        )
